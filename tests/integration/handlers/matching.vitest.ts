...
describe("recomputeMatch - Integration Tests", () => {
  ...
  it("should keep status open when fewer than 5 confirmed entries", async () => {
    ...
    // 3 confirmed (5 人未満)
    await insertEntry(recruitId, "user1", "2026-01-18T11:00:00.000Z");
    await insertEntry(recruitId, "user2", "2026-01-18T11:30:00.000Z");
    await insertEntry(recruitId, "user3", "2026-01-18T12:00:00.000Z");
  ...
  it("should set status matched when 5+ confirmed entries", async () => {
    ...
    for (let i = 1; i <= 5; i++) {
      await insertEntry(recruitId, `user${i}`, baseTime);
    }
  ...
  it("should ping prev members except the trigger on cancellation", async () => {
    ...
    for (const id of ["user1", "user2", "user4", "user5"]) {
      await insertEntry(recruitId, id, baseTime);
    }
  ...
  it("時刻のみ変更時は【更新】通知を1回だけ送り、取消通知は送らない", async () => {
    ...
    for (const id of members) {
      await insertEntry(recruitId, id, id === "user3" ? updatedTime : baseTime);
    }
  ...
  describe("希望パーティサイズによるマッチング制約", () => {
    ...
    it("5人中1人が up_to_trio の場合は status open のまま（フルマッチ対象が5人未満）", async () => {
      const { recruitId } = await setupBase();
      const baseTime = "2026-01-18T11:00:00.000Z";
      for (let i = 1; i <= 4; i++) {
        await insertEntry(recruitId, `user${i}`, baseTime);
      }
      // 5人目は up_to_trio（フルマッチに入らない）
      await insertEntry(recruitId, "user5", baseTime, "up_to_trio");
    ...
    it("full_party/any が5人以上いればその中から5人で matched し、up_to_trio は外れる", async () => {
      const { recruitId } = await setupBase();
      const baseTime = "2026-01-18T11:00:00.000Z";
      for (let i = 1; i <= 5; i++) {
        await insertEntry(recruitId, `user${i}`, baseTime);
      }
      await insertEntry(recruitId, "userT", baseTime, "up_to_trio");
      ...
      const memberIds = JSON.parse(recruit?.matchedMemberIdsJson ?? "[]") as string[];
      expect(memberIds).toHaveLength(5);
      expect(memberIds).not.toContain("userT");
    });
  });
  ...
  describe("5人マッチの早期サブ組併記", () => {
    ...
    it("全員集合より早く始められるランク適合の組があれば確定 embed に併記する", async () => {
      ...
      for (let i = 1; i <= 3; i++) {
        await insertEntry(recruitId, `user${i}`, early);
        await insertRiotAccount(`user${i}`, "Gold 1");
      }
      for (let i = 4; i <= 5; i++) {
        await insertEntry(recruitId, `user${i}`, late);
    ...
    it("全員が同時刻なら早期サブ組は併記しない", async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        await insertEntry(recruitId, `user${i}`, same);
        await insertRiotAccount(`user${i}`, "Gold 1");
      }
